...
class CheckPi(Check):
    ...
    def _cdf_vectorized_impl(self, points):
        ...
        cell_fractions = np.prod(fractions, axis=2)  # Shape: (n_cells, n_points_to_compute)
        
        # Get cell masses
        cell_masses = self.matr.ravel()[:, np.newaxis]  # Shape: (n_cells, 1)
        
        # Calculate weighted sum for each point
        weighted_fractions = cell_masses * cell_fractions  # Shape: (n_cells, n_points_to_compute)
