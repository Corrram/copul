...
import numpy as np
import sympy as sp
from sympy.solvers import solve # May need for cubic if not using explicit form
...
class XiMaxRhoSCopula(BivCopula):
    ...
    u, v = sp.symbols("u v", positive=True, real=True) # u,v in (0,1) typically for CDF evaluation
    ...
    def _validate_rho_s(rho_s_val):
        ...
        if not (0 <= rho_s_val <= 1):
            raise ValueError(f"Parameter rho_s must be in [0,1] for this copula definition, got {rho_s_val}")
    ...
    def _b_star_expr(rho_s_param):
        ...
        """
        b_star_trig = (sp.Rational(5,3)) * sp.cos( (sp.Rational(1,3)) * sp.acos(1 - (sp.Rational(108,25)) * rho_s_param) + (sp.Rational(4,3))*sp.pi ) + sp.Rational(5,6)
        return b_star_trig
    ...
    def _xi_optimal_expr(rho_s_param):
        ...
        xi_opt = sp.Piecewise(
            ...
            (xi_B, rho_s_param <= rho_s_transition),# Includes rho_s=0.3, b_star=1, xi=0.3
    ...
    def _s_expr(v_sym, mu_param):
        ...
        s_for_small_mu = sp.Piecewise( # mu_param <= 1
            (s1_small_mu, v_sym <= v_thresh_small_mu),
            (s2_small_mu, v_sym <= 1 - v_thresh_small_mu), # Corrected condition from example
            (s3_small_mu, True),
        )
        ...
        v1_L = sp.Rational(1,2) * mu_param # This was v1_L = 1/(2*b) if b was slope. If b is mu, then it's mu/2.
        ...
        s_v_mu_ge_1 = sp.Piecewise(
            (sp.sqrt(2 * v_sym / (1/mu_param) ), v_sym <= (1/mu_param) / 2), # using b_slope = 1/mu_param
            (v_sym / (1/mu_param) + sp.Rational(1,2), v_sym <= 1 - (1/mu_param)/2 ),
            (1 + mu_param - sp.sqrt(2 * mu_param * (1-v_sym)), True) # this used mu directly
        )
        ...
        s_final_small_mu = sp.Piecewise(
            (s_piece1_small_mu, v_sym <= v_boundary1_small_mu),
            (s_piece2_small_mu, v_sym <= 1 - v_boundary1_small_mu), # v_boundary2_small_mu
            (s_piece3_small_mu, True),
        )

        # For mu_param > 1 (large mu)
        v_boundary1_large_mu = 1 / (2 * mu_param) # This is b_slope / 2 if mu_param = 1/b_slope. No, this is mu_lit / 2 for the other regime if mu_lit = 1/mu_param.
        ...
        s_piece2_large_mu = v_sym * mu_param + sp.Rational(1,2) # This is v_sym / b_slope + 1/2
    ...
    def cdf(self):
        ...
        cdf_piece_av_neg = (s_v_expr * u_sym - u_sym**2 / 2) / mu_val # for 0 <= u <= s_v
        
        # Case 2: a_v_calc >= 0 (s_v >= mu_val) "Plateau + triangle"
        # Here a_v_calc is the actual $a_v$ from formula.
        middle_piece_av_pos = a_v_calc + (s_v_expr * (u_sym - a_v_calc) - (u_sym**2 - a_v_calc**2) / 2) / mu_val

        # Define threshold $a_{eff} = \max(0, a_v_calc)$ for u-conditions
        a_eff = sp.Max(0, a_v_calc)

        C_expr = sp.Piecewise(
            (u_sym, u_sym <= a_v_calc), # This branch only if a_v_calc >= 0 and u_sym <= a_v_calc
                                     # If a_v_calc < 0, this is never met for u_sym >= 0.
            (middle_piece_av_pos, u_sym <= s_v_expr), # This applies if a_v_calc >=0 and a_v_calc < u_sym <= s_v_expr
