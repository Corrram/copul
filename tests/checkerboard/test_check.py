...
def test_from_data_non_uniform_result():
    """Test that data outside [0, 1] raises an error."""
    # Create data outside [0, 1]
    data = np.array([[1.5, 0.5], [0.5, 1.5]])
    
    with pytest.raises(ValueError):
        Check.from_data(data, already_uniform=True)  # This should raise an error
