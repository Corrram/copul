...
class TestXiNCalculate:
    ...
    def test_nan_values(self):
        """Test Xi_n with NaN values."""
        x = np.array([1, 2, np.nan, 4, 5])
        y = np.array([1, 2, 3, 4, 5])

        # The function appears to handle NaN values by returning 0.5
        result = xi_ncalculate(x, y)
        assert result == pytest.approx(0.5)
...
class TestXiNVarCalculate:
    ...
    def test_empty_vectors(self):
        """Test xi_nvarcalculate with empty vectors."""
        x = np.array([])
        y = np.array([])
    ...
    def test_single_element_vectors(self):
        """Test xi_nvarcalculate with single-element vectors."""
        x = np.array([1])
        y = np.array([2])

        # Test if function returns NaN or 0 for single-element vectors
        result = xi_nvarcalculate(x, y)
        assert np.isnan(result) or result == 0
