...
if __name__ == "__main__":
    ...
    print("Tracing the boundary for (psi, rho) by minimizing -mu*psi + rho...")
    for mu in tqdm(mu_values):
        psi, rho, _ = get_boundary_point_neg_mu_psi_rho(mu, n=32)
    ...
    mu_for_files = [0.05, 0.2, 0.5, 0.8, 1.0, 2, 5, 10.0]
    n_vis = 64  # Use a higher resolution for visualization

    for mu_val in mu_for_files:
        # Compute the optimal H matrix
        _, _, H_map = get_boundary_point_neg_mu_psi_rho(mu=mu_val, n=n_vis)

        if H_map is not None:
            # --- Save to CSV ---
            filename = f"h_matrix_neg_mu_psi_rho_mu_{mu_val:.2f}.csv"
            ...
            ax.set_title(f"Structure of h(t,v) for μ = {mu_val:.2f} (-μψ+ρ)")
            ax.set_xlabel("v")
            ax.set_ylabel("t")
            plt.colorbar(
                im,
