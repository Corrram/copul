...
def d_num(q, c, *, W_func):
    ...
    """
    rhs = (c + 2.0) / 6.0
    f   = lambda d: W_func(d) - rhs          # <-- only d !
...
def C_num(u, v, q, d, *, sv_func):
    if u == 0: return 0.0
    return quad(h_star, 0, u, args=(v,q,d),
                kwargs={"sv_func":sv_func}, epsabs=1e-10)[0]

def C_an(u, v, q, d):
    # wedge/pyramid trick:   C(u,v)=I(u,d)+I(v,d)−I(min(u,v),d)
    # where I(x,d)=∫_0^x h*_x(t)dt  (mass below diagonal)
    def I(x):                   # we still need sv formula but closed-form
        return quad(h_star, 0, x, args=(x,q,d),
                    kwargs={"sv_func":sv_an})[0]
    return I(u) + I(v) - I(min(u,v))
...
def J_an(q, d):
    ...
    return (q**3/20)*d**4 + (2*q**3/15-q**2/6)*d**3 + \
           (q**3/10 - q**2/4)*d**2 - q**3/60 + q**2/12 - q/2 + 2/3
...
def J_num(q, d, *, sv_func, nodes=41):
    ...
    integrand = 2*(1-t)*h_star(t, xs[:,None], q, d, sv_func=sv_func) + (-q)*\
                h_star(t, xs[:,None], q, d, sv_func=sv_func)**2
...
def compute_all(a, c,
    ...
    if verbose: print(f"  d* = {d_star:.6f}")
    C_func = (lambda u,v: C_an(u,v,q,d_star)) if use_C_an else \
             (lambda u,v: C_num(u,v,q,d_star, sv_func=sv))
    psi_val = psi_an(q,c) if (use_C_an and use_d_an and use_W_an) else \
              psi_num(q,d_star, sv_func=sv)
    if use_J_an:
        Jv = J_an(q,d_star)
    else:
