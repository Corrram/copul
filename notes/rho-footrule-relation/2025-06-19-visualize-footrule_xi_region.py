...
ax.fill_between(xi_grid, psi_lo, psi_up, color=FILL, alpha=0.7, label="attainable")

# ---------- optimal point  (xi*, psi*)
ax.scatter([xi_star], [psi_star], s=90, color=RED, zorder=6, label=r"$b^{\star}=0.5$")
ax.annotate(
    r"$(\xi^{\star},\psi^{\star})$",
    (xi_star, psi_star),
    xytext=(12, 8),
...
ax.set_ylabel(r"Spearman's foot-rule $\psi$ (= Gini’s $\gamma$)", fontsize=16)
