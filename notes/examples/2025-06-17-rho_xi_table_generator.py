...
def maximize_rho_minus_xi(x: np.ndarray, rho: np.ndarray, xi: np.ndarray) -> tuple[float, float, float]:
    """
...
def main() -> None:
    ...
    rows.append(("Gaussian", p_gauss, rho_gauss, xi_gauss,
                 rho_gauss - xi_gauss))
    ...
    for fam, p, r_, x_, diff in rows_sorted:
        print(f"    {fam:14s} & {p:10.3f} & {r_:10.3f} & {x_:10.3f} & {diff:10.3f} \\\\")
    print(r"    \bottomrule")
    print(r"  \end{tabular}")

    # --------------- caption --------------------------------------------
    print(r"  \caption{%")
    print(r"  Parameter values that approximately maximise the gap "
          r"$\rho-\xi$ for the listed copula families, together with the ")
    print(r"  corresponding Spearman's~$\rho$, Chatterjee's~$\xi$, and their "
