"""Upper boundary of the (ρ, γ) attainable region via LP.

For each fixed ρ target, maximizes γ(H) subject to ρ(H) == ρ_target
and the standard copula-derivative constraints.  Both measures are linear
in H, so this is a pure Linear Program.
"""
...
def solve_max_gamma_given_rho(rho_target, n=32, verbose=False):
    """LP: maximize γ(H) subject to ρ(H) == rho_target + standard constraints."""
    ...
    problem = cp.Problem(cp.Maximize(gamma_part), constraints)
    try:
...
if __name__ == "__main__":
    ...
    plot_boundary(
        pts,
        xlabel=r"Spearman's $\rho$",
        ylabel=r"Gini's $\gamma$",
        title=r"Upper Boundary of Attainable Region $(\rho, \gamma)$",
        xlim=(-1.1, 1.1),
        ylim=(-1.1, 1.1),
        extra_plots=[
            lambda ax: ax.plot([-1, 1], [-1, 1], "k--", alpha=0.3, label="Identity")
        ],
    )

    for r_val in [-0.9, -0.5, -0.2, 0.0, 0.2, 0.5, 0.9]:
        _, gamma_got, H_map = solve_max_gamma_given_rho(r_val, n=64)
        if H_map is not None:
            plot_h_matrix(H_map, f"h(t,v): ρ={r_val:.2f} → γ={gamma_got:.2f} (max γ)")
    plt.show()
