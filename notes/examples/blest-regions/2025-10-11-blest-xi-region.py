#!/usr/bin/env python3
"""
Plot Spearman's ρ (y-axis) versus Blest's ν (x-axis)  [axes: ν=x, ρ=y].

Upper boundary traced by the V-threshold family C_μ (μ ∈ [0, 2]).
Explicit closed form for ρ ∈ [0,1]: ν_max(ρ) = 1 − (3/4)·(1−ρ)^(4/3).
...
def nu_from_mu(mu: float) -> float:
    return (
        1.0 - 0.75 * mu**4 if mu <= 1.0 else -0.75 * mu**4 + 4 * mu**3 - 6 * mu**2 + 3
    )
...
def main() -> None:
    ...
    ax.plot(
        nu_sorted, rho_sorted, color=BLUE, lw=2.5, label=r"Upper boundary ($\nu(\rho)$)"
    )
    ax.plot(nu_lower, rho_sorted, color=BLUE, lw=2.5)
    ax.fill_betweenx(
        rho_sorted,
        nu_lower,
        nu_sorted,
