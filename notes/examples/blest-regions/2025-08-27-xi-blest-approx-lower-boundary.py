...
def get_boundary_point(mu, n=32, verbose=False):
    H_opt = solve_h(
        lambda H: cp.Minimize(xi_expr(H, n) + mu * nu_expr(H, n)), n, verbose=verbose
    )
    if H_opt is None:
...
if __name__ == "__main__":
    ...
    pts = sweep_boundary(
        get_boundary_point, mu_values, label="lower boundary for (ξ, ν)"
    )
    ...
    plot_boundary(
        ...
        extra_plots=[
            lambda ax: ax.plot(
                x_ref, np.sqrt(x_ref), "k--", label=r"Upper: $\nu=\sqrt{\xi}$"
            )
        ],
    )
    plot_h_matrices(
