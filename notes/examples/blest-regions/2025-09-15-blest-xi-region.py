...
Upper boundary ν_max(ξ) traced by clamped-parabola family C_μ (μ > 0).
...
def main() -> None:
    ...
    ax.plot(-nu_sorted, xi_sorted, color=BLUE, lw=2.5, label=r"$\nu_{\min}(\xi)$")
    ax.fill_betweenx(xi_sorted, -nu_sorted, nu_sorted,
                     color=FILL, alpha=0.7, zorder=0, label="Attainable region")

    mark_key_copulas(ax, W=(-1.0, 1.0), W_xytext=(10, 0), W_ha="left", W_va="top",
                     Pi_xytext=(0, 20), Pi_va="top")

    scatter_annotate(ax, nu_mu(1.0), xi_mu(1.0), r"$C_{\mu=1}$",
                     xytext=(8, -5), ha="left", va="top", fontsize=16)

    apply_standard_axes(ax, r"Blest's $\nu$", r"Chatterjee's $\xi$",
                        ylim=(-0.05, 1.05), major_y=0.1)
    ax.legend(loc="lower right", fontsize=12, frameon=True)
    save_region_plot(fig, "images/xi-blest-region.png")
