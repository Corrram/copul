...
def get_boundary_point_psi_nu_upper(mu, n=32, verbose=False, solver="OSQP"):
    """
    Upper boundary of (psi, nu): minimize  mu * psi(H) - nu(H)  over feasible H.
...
if __name__ == "__main__":
    ...
    print("Tracing the upper boundary for (ψ, ν)...")
    for mu in tqdm(mu_values):
        psi, nu, _ = get_boundary_point_psi_nu_upper(mu, n=32)
        if psi is not None:
    ...
    if boundary_points:
        ...
        plt.plot(
            ...
            boundary_points[:, 1],
            "o-",
            label="Numerical Upper Boundary (ψ–ν)",
        )

    # reference anchors (common for many copula metrics)
    plt.plot([0], [0], "ks", label="Independence (ψ=0, ν=0)")
    plt.plot([1], [1], "k^", label="Comonotone (ψ=1, ν=1)")

    plt.title("Attainable Region (Upper Boundary) for (ψ, ν)")
    ...
    mu_for_files = [0.05, 0.1, 0.2, 0.5, 1.0, 1.2, 1.5, 2.0, 5.0, 10.0]
    n_vis = 64

    for mu_val in mu_for_files:
        _, _, H_map = get_boundary_point_psi_nu_upper(mu=mu_val, n=n_vis)
        if H_map is not None:
            ...
            ax.set_title(f"h(t,v) for μ = {mu_val:.2f} (ψ–ν Upper Boundary)")
            ax.set_xlabel("v")
            ax.set_ylabel("t")
            plt.colorbar(
                im,
