"""Upper boundary of the (ν, γ) attainable region.
...
def get_boundary_point(mu, n=32, verbose=False):
    ...
    H_opt = solve_h(
        lambda H: cp.Minimize(mu * nu_expr(H, n) - gamma_expr(H, n, M, J)),
        n,
        verbose=verbose,
    )
    if H_opt is None:
...
if __name__ == "__main__":
    pts = sweep_boundary(get_boundary_point, label="upper boundary for (ν, γ)")
    plot_boundary(
        pts,
        xlabel=r"Blest's $\nu$",
        ylabel=r"Gini's $\gamma$",
        title="Attainable Region (Upper Boundary) for (ν, γ)",
        xlim=(-0.05, 1.05),
        ylim=(-1.05, 1.05),
    )
    plot_h_matrices(
