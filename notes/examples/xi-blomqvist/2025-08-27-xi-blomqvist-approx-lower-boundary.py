"""Lower boundary of the (ξ, β) attainable region.
...
if __name__ == "__main__":
    pts = sweep_boundary(get_boundary_point, label="lower boundary for (ξ, β)")
    plot_boundary(
        pts,
        xlabel=r"Chatterjee's $\xi$",
        ylabel=r"Blomqvist's $\beta$",
        title="Attainable Region (Lower Boundary) for (ξ, β)",
        xlim=(-0.05, 1.05),
        ylim=(-1.05, 1.05),
    )
    plot_h_matrices(
