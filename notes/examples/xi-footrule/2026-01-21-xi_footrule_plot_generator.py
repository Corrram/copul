...
def _calc_cmu_point(mu, grid_size=200):
    ...
    def C_uu(u_val):
        ...
        res, _ = quad(
            lambda s: min(beta, max(0, t - _psi_s(s, alpha, beta))),
            0,
            u_val,
...
def main():
    ...
    plt.plot(
        xi_jl, phi_jl, color=COLORS["JensenBound"], lw=2.5, label=r"$C^{\searrow}_\mu$"
    )
    ...
    for fam in FAMILIES:
        ...
        xi_raw, phi_raw = load_family_data(
            ...
            data_dir,
            key1="chatterjees_xi",
            key2="spearmans_footrule",
        )
        if xi_raw is None:
            # fallback key name
            xi_raw, phi_raw = load_family_data(
                fam, data_dir, key1="chatterjees_xi", key2="phi"
            )
