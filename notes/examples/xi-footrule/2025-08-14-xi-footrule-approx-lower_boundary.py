...
def get_boundary_point(mu, n=32, verbose=False):
    M = M_lower(n)
    H_opt = solve_h(
        lambda H: cp.Minimize(psi_expr(H, n, M) + mu * xi_expr(H, n)),
        n,
        marginal_offset=0.5,
        verbose=verbose,
    )
...
if __name__ == "__main__":
    ...
    pts = sweep_boundary(
        get_boundary_point, mu_values, label="lower boundary for (ξ, ψ)"
    )

    x_ref = np.linspace(0.001, 1, 100)
    plot_boundary(
        pts,
        xlabel=r"Chatterjee's $\xi$",
        ylabel=r"Spearman's footrule $\psi$",
        title="Attainable Region (Lower Boundary) for (ξ, ψ)",
        xlim=(-0.1, 1.05),
        ylim=(-0.55, 1.05),
        extra_plots=[
            lambda ax: ax.plot(
                x_ref, np.sqrt(x_ref), "r--", label=r"Upper bound $\psi=\sqrt{\xi}$"
            )
        ],
    )
    plot_h_matrices(
