...
def b_from_x_regime1(x_val):
    ...
    if x_val <= 3/10: # Should be handled by regime 2
        if np.isclose(x_val, 3/10): return 1.0
        return np.nan
...
# --- Create the plot ---
fig, ax = plt.subplots(figsize=(6, 8)) # Adjusted figsize to be taller, closer to example

# Plot the corrected M_x upper bound
ax.plot(xi_coords_valid, rho_upper_valid, label=r'$M_\xi$ (Upper Bound)', color='#00529B', linewidth=2.5, zorder=3)

# Plot the mirrored lower bound -M_x
ax.plot(xi_coords_valid, rho_lower_mirrored, label=r'$-M_\xi$ (Implied by Symmetry)', color='#00529B', linestyle='--', linewidth=2.5, zorder=3)

# Mark key points (without text labels as in example)
key_points_xi = [0, 1, 0]
key_points_rho = [0, 1, -1]
ax.scatter(key_points_xi, key_points_rho, color='black', s=60, zorder=5) # Slightly smaller points

# Fill the attainable region between -M_x and M_x
ax.fill_between(xi_coords_valid, rho_lower_mirrored, rho_upper_valid,
                 where=rho_upper_valid >= rho_lower_mirrored,
                 color='#D6EAF8', alpha=0.7, interpolate=True, label=r'Attainable Region', zorder=1) # Adjusted color and alpha
...
legend = ax.legend(loc='center', ncol=1, fontsize=12, frameon=True, fancybox=False, shadow=False, borderpad=0.5)
