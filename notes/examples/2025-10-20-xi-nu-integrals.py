...
F = sp.Lambda((sp.Symbol('x'), sp.Symbol('qq')),  sp.Rational(1,5)*sp.Symbol('x')**5 - sp.Rational(2,3)*sp.Symbol('qq')*sp.Symbol('x')**3 + sp.Symbol('qq')**2*sp.Symbol('x'))
S = sp.Lambda((sp.Symbol('x'), sp.Symbol('qq')),  sp.Rational(1,5)*sp.Symbol('x')**5 - sp.Rational(1,3)*sp.Symbol('qq')*sp.Symbol('x')**3)
...
# ------------------------------------------------------------
# Case A: 0<b<=1
Xi_A  = sp.simplify( 6*( sp.integrate(G_i*w_i,  (R, 0, 1))
                       + sp.integrate(G_ii*w_ii,(R, 1, 1/sp.sqrt(b)))
                       + sp.integrate(G_iv*w_iv,(r, 0, 1)) ) - 2 )
Nu_A  = sp.simplify( 12*( sp.integrate(H_i*w_i,  (R, 0, 1))
                        + sp.integrate(H_ii*w_ii,(R, 1, 1/sp.sqrt(b)))
                        + sp.integrate(H_iv*w_iv,(r, 0, 1)) ) - 2 )

# Case B: b>1
Xi_B  = sp.simplify( 6*( sp.integrate(G_i*w_i,   (R, 0, 1/sp.sqrt(b)))
                       + sp.integrate(G_iii*w_iii,(R, 1/sp.sqrt(b), 1))
                       + sp.integrate(G_iv*w_iv,  (r, sp.sqrt(1-1/b), 1)) ) - 2 )
Nu_B  = sp.simplify( 12*( sp.integrate(H_i*w_i,   (R, 0, 1/sp.sqrt(b)))
                        + sp.integrate(H_iii*w_iii,(R, 1/sp.sqrt(b), 1))
                        + sp.integrate(H_iv*w_iv,  (r, sp.sqrt(1-1/b), 1)) ) - 2 )
...
print("\nCase 0<b<=1:")
print("xi(C_b) =");  sp.pprint(Xi_A)
print("nu(C_b) =");  sp.pprint(Nu_A)

print("\nCase b>1:")
print("xi(C_b) =");  sp.pprint(Xi_B)
print("nu(C_b) =");  sp.pprint(Nu_B)

gamma = sp.sqrt((b-1)/b)

A = sp.acosh(sp.sqrt(b))

Xi_target = (183*gamma - 38*b*gamma - 88*b**2*gamma + 112*b**2
             + 48*b**3*gamma - 48*b**3 - 105*A/b) / 210

Nu_target = ((87*gamma)/b + 250*gamma - 376*b*gamma + 448*b
             + 144*b**2*gamma - 144*b**2 - 105*A/b**2) / 420

# Help SymPy see acosh: replace logs with acosh form
R = sp.sqrt(b*(b-1))
Xi_rewritten = Xi_B.xreplace({
    sp.log(b) - 2*sp.log(b + R): -2*sp.acosh(sp.sqrt(b))
})
Nu_rewritten = Nu_B.xreplace({
    sp.log(b) - 2*sp.log(b + R): -2*sp.acosh(sp.sqrt(b))
})
