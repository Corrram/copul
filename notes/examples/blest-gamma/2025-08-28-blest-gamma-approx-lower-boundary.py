...
def get_boundary_point_nu_gamma_lower(mu, n=32, verbose=False, solver="OSQP"):
    """
    Solve the discretized problem for a given mu:
        minimize_H   mu * nu(H) + gamma(H)
    to trace the LOWER boundary of the (nu, gamma) region.
...
if __name__ == "__main__":
    ...
    print("Tracing the lower boundary for (nu, gamma)...")
    for mu in tqdm(mu_values):
        nu, gamma, _ = get_boundary_point_nu_gamma_lower(mu, n=32)
        if nu is not None:
    ...
    if boundary_points:
        ...
        plt.plot(
            ...
            boundary_points[:, 1],
            "o-",
            label="Numerical Lower Boundary (ν–γ)",
        )

    plt.title("Attainable Region (Lower Boundary) for (ν, γ)")
    ...
    for mu_val in mu_for_files:
        _, _, H_map = get_boundary_point_nu_gamma_lower(mu=mu_val, n=n_vis)
        if H_map is not None:
            ...
            ax.set_title(f"Structure of h(t,v) for μ = {mu_val:.2f} (γ Lower Bound)")
            ax.set_xlabel("v")
            ax.set_ylabel("t")
            plt.colorbar(
                im,
