...
def b_from_x(x: float) -> float:
    ...
    if 0 < x < 0.3:  # (0, 3/10)
        ...
        try:
            ...
            b_val = brentq(lambda b_var: 2 * b_var**3 - 5 * b_var**2 + 10 * x, 1e-10, 1.0)
            return b_val
...
def extremal_copula(u, v, x=0.5):
    ...
    if b >= 1.0: # This includes b=1.0 where x=0.3
        ...
        # Mask for v values
        mask_v_le_one_over_2b = (v_arr <= one_over_2b + 1e-12) # Add tolerance for floating point
        ...
        s[current_mask] = np.sqrt(np.maximum(0, 2.0 * v_arr[current_mask] / b)) # ensure sqrt non-negative
        ...
        if b > 1.0 + 1e-12: # Strictly b > 1
            ...
            mask_c2 = np.logical_not(np.logical_or(mask_v_le_one_over_2b, mask_v_gt_one_minus_one_over_2b))
    ...
    # Conditions for 0 < b < 1.0
        ...
        mask_c3_b_lt_1 = np.logical_not(np.logical_or(mask_v_le_b_over_2, mask_v_gt_one_minus_b_over_2))
    ...
    if np.any(tri_mask):
        ...
        if np.any(sub2): C_tri_vals[sub2] = v_arr[tri_mask][sub2]
    ...
    if np.any(plat_mask):
        ...
        mask_u_gt_a_le_s = np.logical_and(u_plat > a_plat_safe - 1e-12, u_plat <= s_plat + 1e-12)
        if np.any(mask_u_gt_a_le_s):
            ...
            C_plat_vals[mask_u_gt_a_le_s] = (
                ...
                + b * (s2 * (u2 - a2) - 0.5 * (u2**2 - a2**2)) # Integral of ramp b(s2-t) from a2 to u2
            )
    ...
    C = np.minimum(C, np.broadcast_to(v_arr, C.shape)) # v_arr might need broadcasting if u/v shapes differ
...
if __name__ == "__main__":
    ...
    for x_val in x_values_to_test:
        ...
        try:
            ...
            print(f"τ (Kendall) = {tau:.6f}   ξ (xi) = {xi:.6f}   ρ (Spearman) = {rho:.6f}")
