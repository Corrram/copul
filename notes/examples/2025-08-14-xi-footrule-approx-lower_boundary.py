...
def get_boundary_point(mu, n=32, verbose=False):
    ...
    problem.solve(solver=cp.OSQP, verbose=verbose, max_iter=20000, eps_abs=1e-5, eps_rel=1e-5)
...
if __name__ == '__main__':
    ...
    plt.plot(boundary_points[:, 0], boundary_points[:, 1], 'o-', label='Numerical Lower Bound')
    ...
    for mu_val in mu_for_files:
        ...
        if H_map is not None:
            # --- SAVE TO CSV ---
            # Create a descriptive filename, e.g., "h_matrix_mu_1.00.csv"
            filename = f"h_matrix_mu_{mu_val:.2f}.csv"

            # Save the matrix to a CSV file, with comma delimiters
            ...
            ax = plt.gca()
            im = ax.imshow(H_map, origin='lower', extent=[0, 1, 0, 1], cmap='viridis', vmin=0,
                           vmax=1)
